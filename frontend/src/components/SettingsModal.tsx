        ...
        <nav className="w-[200px] min-w-[200px] bg-white/[0.01] border-r border-white/[0.03] flex flex-col p-3 gap-0.5">
          <div className="flex items-center justify-between px-3 pb-3 mb-2 border-b border-white/[0.03]">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-serif text-sm font-[450] text-text-main">Configuración</span>
            </div>
            <button
          ...
          {tabs.map(tab => (
            <button
              key={tab.id}
              className={`flex items-center gap-2.5 bg-none border-none text-text-sub px-3 py-[9px] w-full rounded-xl cursor-pointer text-[12px] font-[400] text-left transition-all hover:bg-white/[0.02] hover:text-text-main ${
        ...
        <div className="flex-1 p-8 overflow-y-auto max-h-[80vh] scrollbar-thin">
          {settingsTab === 'general' && (
            <div className="flex flex-col gap-5 max-w-[520px]">
              <h3 className="font-serif text-lg font-[400] text-text-main tracking-[-0.01em]">General</h3>
              <div className="bg-white/[0.01] border border-white/[0.04] rounded-xl p-5">
                <p className="text-sm text-text-sub font-[350] leading-relaxed">
          ...
          {settingsTab === 'train' && (
            <div className="flex flex-col gap-5 max-w-[520px]">
              <h3 className="font-serif text-lg font-[400] text-text-main tracking-[-0.01em]">Entrenar Sententia</h3>
              <div className="bg-accent/[0.02] border border-accent/8 rounded-xl p-5">
                <p className="text-sm text-text-sub font-[350] leading-relaxed">
                  ...
                  Sententia soporta archivos <span className="text-text-main font-[400]">PDF, TXT y MD</span>.
                </p>
              </div>
              <div className={`border-2 border-dashed border-white/[0.04] rounded-xl p-8 text-center transition-all cursor-pointer flex flex-col items-center gap-3 relative hover:border-accent/20 hover:bg-accent/[0.01] ${
                ...
                {trainingFile ? (
                  <span className="font-[400] text-accent text-sm break-all">{trainingFile.name}</span>
                ) : (
                  <div>
                    <p className="text-sm text-text-sub font-[350]">Haga clic para seleccionar archivo</p>
                    <small className="text-[11px] text-text-muted/50 font-[350]">PDF, TXT o MD</small>
                  </div>
                )}
              </div>
              <button
                className="w-full py-3 rounded-xl bg-accent/8 border border-accent/15 text-accent font-[450] cursor-pointer transition-all hover:bg-accent/12 hover:border-accent/22 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2.5 text-sm"
              ...
              {resolvedTrainingStatus && (
                <div className={`text-sm text-center p-3.5 rounded-xl font-[350] ${
          ...
          {settingsTab === 'customization' && (
            <div className="flex flex-col gap-5 max-w-[520px]">
              <h3 className="font-serif text-lg font-[400] text-text-main tracking-[-0.01em]">Personalización</h3>
              <div className="bg-white/[0.01] border border-white/[0.04] rounded-xl p-5">
                <p className="text-sm text-text-sub font-[350] leading-relaxed">
          ...
          {settingsTab === 'hidden' && (
            <div className="flex flex-col gap-4 max-w-[520px]">
              <h3 className="flex items-center gap-2 font-serif text-lg font-[400] text-text-main tracking-[-0.01em]">
              ...
              <button
                className="w-full py-2.5 rounded-xl bg-accent/8 border border-accent/12 text-accent font-[450] cursor-pointer transition-all hover:bg-accent/12 hover:border-accent/18 flex items-center justify-center gap-2 text-sm"
              ...
              <div className="flex flex-col gap-1 max-h-[240px] overflow-y-auto scrollbar-thin mt-2">
                {chats.filter(c => c.isHidden).length === 0 ? (
                  <p className="text-sm text-text-muted/50 font-[350]">No hay chats ocultos.</p>
                ) : (
                  chats.filter(c => c.isHidden).map(chat => (
                    <div key={chat.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.01] border border-white/[0.03]">
                      ...
                      <div className="flex gap-1.5 shrink-0">
                        <button
                          className="px-3 py-1.5 rounded-lg bg-accent/8 border border-accent/12 text-accent text-[11px] font-[450] cursor-pointer transition-all hover:bg-accent/12 hover:border-accent/18"
                          onClick={() => onRestoreChat(chat.id)}>
                          Mostrar
                        </button>
                        <button
                          className="px-2 py-1.5 rounded-lg border border-danger/12 text-danger/50 text-[11px] cursor-pointer transition-all hover:bg-danger/6"
